...
class OWSubstructureSearch(widget.OWWidget):
    ...
    def apply_search(self):
        ...
        try:
            # Convert the query SMILES/SMARTS to an RDKit molecule
            query_mol = Chem.MolFromSmarts(smiles_query) if SEARCH_TYPES[self.search_type] != "exact" else Chem.MolFromSmiles(smiles_query)
            if query_mol is None:
                raise ValueError("Invalid SMILES/SMARTS string.")
        except Exception as e:
            self.error(f"Error parsing query: {e}")
            return

        # Filter the input data based on the search type
        filtered_data = []
        highlighted_atoms = []
        ...
        for row in self.data:
            ...
            match = self.match_molecule(mol, query_mol, SEARCH_TYPES[self.search_type])
            if match:
                filtered_data.append(row)
                highlighted_atoms.append(",".join(map(str, mol.GetSubstructMatch(query_mol))) if SEARCH_TYPES[self.search_type] == "substructure" else "")

        # Create and send the filtered data table
        if filtered_data:
            domain = self.data.domain
            highlighted_var = StringVariable("Highlighted Atoms")
            new_domain = Domain(domain.attributes, domain.class_vars, list(domain.metas) + [highlighted_var])
            meta_data = [list(row.metas) + [highlighted_atoms[i]] for i, row in enumerate(filtered_data)]
    ...
    def match_molecule(self, mol, query_mol, search_type):
        ...
        if search_type == "substructure":
            ...
            return AllChem.DataStructs.TanimotoSimilarity(fp1, fp2) > 0.8
